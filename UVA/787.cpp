...
struct BigInt {
	...
	BigInt(size_t size, char c) :
			digits(size, c), sign(PLUS) {
	}
...
BigInt bigIntMultiPly(const BigInt &a, const BigInt &b) {
	...
	while (c.digits.size() > 1 && c.digits[c.digits.size() - 1] == '0') {
